...
impl<T> MemPool<T> {
  ...
  pub fn new(size: usize) -> Self {
    ...
    for i in 0..size {
      buf.push(UnsafeCell::new(Slot { next: i + 1 }))
    }
  ...
  unsafe fn try_alloc_unchecked_with_head(&self, head: usize) -> Option<*mut T> {
    ...
    let next_head = unsafe { (*self.buf[index].get()).next };
    if self
      .head
      .compare_exchange(
        head,
    ...
    unsafe {
      // `slot` has already been popped from the queue,
      // so no other thread is accessing it now, therefore
      // we can convert this &Slot<T> to &mut Slot<T>
      (*slot.get()).elem = ManuallyDrop::new(MaybeUninit::uninit());
      Some((*(*slot.get()).elem).as_mut_ptr())
    }
  }
  ...
  fn calculate_idx_from_ptr(&self, ptr: *mut T) -> usize {
    let slot = unsafe { Self::slot_from_elem(ptr) } as *const UnsafeCell<Slot<T>>;
  ...
  pub unsafe fn try_free(&self, handle: *mut T) -> bool {
    ...
    let slot = unsafe { Self::slot_from_elem(handle) } as *const UnsafeCell<Slot<T>>;
